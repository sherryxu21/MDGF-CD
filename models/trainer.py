...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        # define G
        self.net_G = define_G(args=args, gpu_ids=args.gpu_ids).to(self.device)
    ...
    def _visualize_pred(self):
        pred = torch.argmax(self.G_final_pred, dim=1, keepdim=True).to(self.device)
        pred_vis = pred * 255
        return pred_vis
    ...
    def _update_metric(self):
        ...
        G_pred = self.G_final_pred.to(self.device).detach()

        G_pred = torch.argmax(G_pred, dim=1).to(self.device)
    ...
    def _update_checkpoints(self):
        ...
        self.logger.write('Lastest models updated. Epoch_acc=%.4f, Historical_best_acc=%.4f (at epoch %d)\n'
              % (self.epoch_acc, self.best_val_acc, self.best_epoch_id))
        self.logger.write('\n')

        # update the best models (based on eval acc)
        if self.epoch_acc > self.best_val_acc:
            self.best_val_acc = self.epoch_acc
            self.best_epoch_id = self.epoch_id
            self._save_checkpoint(ckpt_name='best_ckpt.pt')
            self.logger.write('*' * 10 + 'Best models updated!\n')
            self.logger.write('\n')
    ...
    def _backward_G(self):
        ...
        if self.multi_scale_train == "True":
            ...
            self.G_loss = self._pxl_loss(self.G_pred, gt).to(self.device)
    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            self.net_G.train()  # Set models to training mode
            # Iterate over data.
